...
def image_transformer(image_data):
    logging.info('============= image_transformer')
    # 变换操作定义
    ...
    images = transf(image)
    print(f"after image open, images.shape = {images.shape}")
    print(f"images.shape[0] = {images.shape[0]}")
    if images.shape[0] == 1:
        # img = np.stack((images,)*3, axis=-1)
        # print(f"img.shape = {img.shape}")
        # for tensor_add in range(3):
        img = torch.cat((images, images, images), 0)
        # t = transform(img)
        print(f"++++++++ images.shape = {img.shape}")

    trans = transform(image).unsqueeze_(0)
